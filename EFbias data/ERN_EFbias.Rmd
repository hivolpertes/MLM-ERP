...
ITmat[4,] <- c(1,1,1,0,1,rep(0,3)) # intercept for race = 1, object = 1 (white-tool)
...
SETmat <- matrix(NA,1024,8) # 1024 rows because 256 trials x 4 conditions
# fill columns with appropriate weights.
SETmat[,1] = 1 
SETmat[,2] = rep(c(0, 0, 1, 1), each = 256) # 0 for first two conditions, 1 for second two conditions
SETmat[,3] = rep(c(0, 1, 0, 1), each = 256) 
SETmat[,4] = rep(1:256, 4) # 1:256 in each condition
SETmat[,5] = rep(c(0, 0, 0, 1), each = 256) 
SETmat[,6] = c(rep(c(0, 0), each = 256), rep(1:256, 2)) # 0 for first two conditions, 1:256 for second two conditions
SETmat[,7] = c(rep(0, 256), 1:256, rep(0, 256), 1:256) # 0 for first and third condition, 1:256 for second and last condition
SETmat[,8] = c(rep(c(0, 0, 0), each = 256), 1:256) # 0 for first three conditions, 1:256 for last condition
...
SE.dat = data.frame(Condition = rep(c("Black_gun", "Black_tool", "White_gun", "White_tool"), each = 256), 
                    Trial = rep(c(1:256), 4), 
                    SE = ses)
...
SE.dat$Point = NULL
for (c in c("Black_gun", "Black_tool", "White_gun", "White_tool")) {
  SE.dat$Point[SE.dat$Condition == c] = 
    forPlotting$Intercept[forPlotting$Condition == c] + 1:256*forPlotting$Slope[forPlotting$Condition == c]
}
...
# BINGO -------------------------------------------------------------------
ggplot(SE.dat, aes(Trial, Point, alpha = Condition, color = Condition, shape = Condition)) +
  geom_abline(data = forPlotting, aes(intercept=Intercept, slope=Slope, color = Condition), size=1.5)+
  ...
  theme_bw() +
  scale_y_continuous(limits =c(-5, 2.5), expand=c(0,0)) +
...
ggsave("SimpleSlopes_TimeOnTask.tiff")
```
...
sum = lmer(MeanAmp ~ (1|Subject) + (1|Electrode), dat = trialOrderDat) %>% summary()

subICC = 8.289/(8.289+2.025+61.041^2)
elecICC = 2.025/(8.289+2.025+61.041^2)
```
...
``` {r}
# check ICCs with signal averaging approach
sigavg = select(trialOrderDat, -Trial, -Race, -Object, -Accuracy, -TrialOrder) %>%
...
subICC = 8.176/(8.176+2.018+6.88^2)
elecICC = 2.018/(8.176+2.018+6.88^2)
```
...
# calculate SE for each trial, separately for each race
SETmat <- matrix(NA,564,8) # 564 rows because 141 errors (max) x 4 conditions
# fill columns with appropriate weights.
SETmat[,1] = 1 
SETmat[,2] = rep(c(0, 0, 1, 1), each = 141) # 0 for first two conditions, 1 for second two conditions
SETmat[,3] = rep(c(0, 1, 0, 1), each = 141) 
SETmat[,4] = rep(1:141, 4) # 1:256 in each condition
SETmat[,5] = rep(c(0, 0, 0, 1), each = 141) 
SETmat[,6] = c(rep(c(0, 0), each = 141), rep(1:141, 2)) # 0 for first two conditions, 1:256 for second two conditions
SETmat[,7] = c(rep(0, 141), 1:141, rep(0, 141), 1:141) # 0 for first and third condition, 1:256 for second and last condition
SETmat[,8] = c(rep(c(0, 0, 0), each = 141), 1:141) # 0 for first three conditions, 1:256 for last condition

newvc <- SETmat %*% vcmat %*% t(SETmat)
ses <- sqrt(diag(newvc)) # long string of SEs for each trial, first 256 are for Black faces, second 256 are for White faces

SE.dat = data.frame(Condition = rep(c("Black_gun", "Black_tool", "White_gun", "White_tool"), each = 141), 
                    Trial = rep(c(1:141), 4), 
                    SE = ses)
...
for (c in c("Black_gun", "Black_tool", "White_gun", "White_tool")) {
  SE.dat$Point[SE.dat$Condition == c] = 
    forPlotting$Intercept[forPlotting$Condition == c] + 1:141*forPlotting$Slope[forPlotting$Condition == c]
}
...
custom = c("dodgerblue", "blue", "red", "red4")
# BINGO -------------------------------------------------------------------
ggplot(SE.dat, aes(Trial, Point, alpha = Condition, color = Condition, shape = Condition, color = linetype)) +
  geom_point()+
  geom_abline(data = forPlotting, aes(intercept=Intercept, slope=Slope, color = Condition, linetype = Condition), size=1.5)+
  geom_ribbon(aes(ymin=lower, ymax=upper, x = Trial, fill = Condition),
              linetype = "dotted",
              alpha = .25,
              color = rep(custom, each=141),
              fill = rep(custom, each=141)) +
  ...
  theme_bw() +
  scale_y_continuous(limits =c(-5, 2.5), expand=c(0,0)) +
  scale_x_continuous(expand=c(0,0), breaks=c(25, 50, 75, 100, 125, 150)) +
...
ggsave("./SimpleSlopes_ErrorNumber.tiff")
```
...
m6 = lmer(MeanAmp ~ Race*Object*TrialOrder + (1|Electrode), dat = trialOrderDat)
m5 = lmer(MeanAmp ~ Race*Object*TrialOrder + (Race*Object|Subject), dat = trialOrderDat)
m4 = lmer(MeanAmp ~ Race*Object*TrialOrder + (Race+Object|Subject), dat = trialOrderDat)
m3 = lmer(MeanAmp ~ Race*Object*TrialOrder + (Race|Subject), dat = trialOrderDat)
m2 = lmer(MeanAmp ~ Race*Object*TrialOrder + (Object|Subject), dat = trialOrderDat)
m1 = lmer(MeanAmp ~ Race*Object*TrialOrder + (1|Subject), dat = trialOrderDat)
m0 = lm(MeanAmp ~ Race*Object*TrialOrder, dat = trialOrderDat)
...
# test slope of object by subject
anova(m2, m1)

# test slope of object*race interaction by subject
anova(m5, m4)
